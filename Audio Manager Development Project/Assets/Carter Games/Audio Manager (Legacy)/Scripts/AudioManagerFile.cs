 ...
 *      Holds data of the audio to be played
 *			
 *  Written by:
 *      Jonathan Carter
 *
...
namespace CarterGames.Legacy.AudioManager
{
    ...
    {
        public string baseScanDirectory;
        ...
        public Dictionary<string, AudioLibrary> Library;

        public List<AudioLibrary> clips; // Moved in 2.3.5 to be here instead of in the AM reference as it caused some issues with the automation.
        ...
        {
            Library = new Dictionary<string, AudioLibrary>();

            if (clips == null || clips.Count <= 0) return;
            foreach (var clip in clips)
                Library.Add(clip.key, clip);
        }
        ...
        public void Play(string request, float volume = 1f, float pitch = 1f)
        {
            ...
            _source = SourceSetup(_source, Library[request].value, 0, volume, pitch);
            _source.Play();
        ...
        public void Play(string request, int mixerID, float volume = 1f, float pitch = 1f)
        {
            ...
            _source.outputAudioMixerGroup = audioMixer[mixerID];
            _source.Play();
        ...
        public void Play(string request, Hashtable args)
        {
            ...
            _source.clip = Library[request].value;
        ...
        public void PlayRange(string[] request, float volume = 1f, float pitch = 1f)
        {
            ...
            _source = SourceSetup(_source, Library[request[Random.Range(0, request.Length)]].value, 0, volume, pitch);
            _source.Play();
        ...
        public void PlayRange(List<string> request, float volume = 1f, float pitch = 1f)
        {
            ...
            _source = SourceSetup(_source, Library[request[Random.Range(0, request.Count)]].value, 0, volume, pitch);
            _source.Play();
        ...
        public void PlayRange(string[] request, Hashtable args)
        {
            ...
            _source = SourceSetup(_source, Library[request[Random.Range(0, request.Length)]].value);
            _source = UpdateSourceWithArgs(_source, args);
            _source.Play();
        ...
        public void PlayRange(List<string> request, Hashtable args)
        {
            ...
            _source = SourceSetup(_source, Library[request[Random.Range(0, request.Count)]].value);
            _source = UpdateSourceWithArgs(_source, args);
            _source.Play();
        ...
        /// <param name="volume">Optional | Float | The volume that the clip will be played at | Default = 1.</param>
        /// <param name="pitch">Optional | Float | The pitch that the sound is played at | Default = 1.</param>
        public AudioSource PlayRangeAndGetSource(string[] request, AudioMixerGroup mixer, float volume = 1f,
            float pitch = 1f)
        {
            if (!canPlayAudio) return null;
        ...
        /// <param name="volume">Optional | Float | The volume that the clip will be played at | Default = 1.</param>
        /// <param name="pitch">Optional | Float | The pitch that the sound is played at | Default = 1.</param>
        public AudioSource PlayRangeAndGetSource(List<string> request, AudioMixerGroup mixer, float volume = 1f,
            float pitch = 1f)
        {
            if (!canPlayAudio) return null;
        ...
        public void PlayFromTime(string request, float time, float volume = 1f, float pitch = 1f)
        {
            ...
            _source = SourceSetup(_source, Library[request].value, time, volume, pitch);
            _source.Play();
        ...
        /// <param name="volume">Float | The volume that the clip will be played at, default = 1</param>
        /// <param name="pitch">Float | The pitch that the sound is played at, default = 1</param>
        public AudioSource PlayFromTimeAndGetSource(string request, float time, AudioMixerGroup mixer,
        ...
        /// <param name="volume">Float | The volume that the clip will be played at, default = 1</param>
        /// <param name="pitch">Float | The pitch that the sound is played at, default = 1</param>
        public AudioSource PlayFromTimeAndGetSource(string request, float time, int mixerID, float volume = 1f,
            float pitch = 1f)
        {
            if (!canPlayAudio) return null;
            if (!HasClip(request)) return null;
        ...
        /// <param name="volume">Float | The volume that the clip will be played at, default = 1</param>
        /// <param name="pitch">Float | The pitch that the sound is played at, default = 1</param>
        public void PlayWithDelay(string request, float delay, AudioMixerGroup mixer, float volume = 1f,
            float pitch = 1f)
        {
            if (!canPlayAudio) return;
            if (!HasClip(request)) return;
        ...
        /// <param name="pitch">Float | The pitch that the sound is played at, default = 1</param>
        /// <returns>AudioSource | The AudioSource that has been set on this method call.</returns>
        public AudioSource PlayWithDelayAndGetSource(string request, float delay, AudioMixerGroup mixer,
            float volume = 1f, float pitch = 1f)
        {
            if (!canPlayAudio) return null;
            if (!HasClip(request)) return null;
        ...
        /// <param name="pitch">Float | The pitch that the sound is played at, default = 1</param>
        /// <returns>AudioSource | The AudioSource that has been set on this method call.</returns>
        public AudioSource PlayWithDelayAndGetSource(string request, float delay, int mixerID, float volume = 1f,
            float pitch = 1f)
        {
            if (!canPlayAudio) return null;
            if (!HasClip(request)) return null;
        ...
        /// </summary>
        public void PlayRandomFromFile(AudioMixerGroup mixerGroup)
        {
            if (!canPlayAudio) return;
            ...
            _source = SourceSetup(_source, Library[GetRandomSound.name].value, 0, 1, 1);
        ...
        public void PlayRandom(float volume = 1, float pitch = 1)
        {
            ...
            _source = SourceSetup(_source, Library[GetRandomSound.name].value, 0, volume, pitch);
            _source.Play();
        ...
        public void PlayRandom(Hashtable args)
        {
            ...
            _source.clip = Library[GetRandomSound.name].value;
            _source = UpdateSourceWithArgs(_source, args);
            _source.time = 0;
            _source.Play();
        ...
        public void PlayRandomFromTime(float time, float volume = 1f, float pitch = 1f)
        {
            ...
            _source = SourceSetup(_source, Library[GetRandomSound.name].value, time, volume, pitch);
        ...
        /// </summary>
        private GameObject ClipSetup => AudioManagerPool.GetPrefabFromPool(soundPrefab);
        ...
        private AudioSource SourceSetup(AudioSource source, AudioClip clip, float time = 0f, float vol = 1f,
            float pit = 1f)
        {
            source.clip = clip;
            source.volume = vol;
        ...
        {
            AudioManagerPool.Active.Add(source);
            removal.Cleanup(this, clip, source.clip.length);
        }
        ...
        public bool HasClip(string request)
        {
            ...
            Debug.LogWarning(
                "* Audio Manager * | Warning Code 2 | Could not find clip. Please ensure the clip is scanned and the string you entered is correct (Note the input is CaSe SeNsItIvE).");
            return false;
        }
        ...
        /// </summary>
        public int GetNumberOfClips(AudioManagerFile file)
        {
        ...
        /// </summary>
        public AudioClip GetRandomSound => clips[Random.Range(0, Library.Count - 1)].value;
        ...
        {
            if (AudioManagerPool.Active == null) return false;
            if (AudioManagerPool.Active.Count == 0) return false;

            for (int i = 0; i < AudioManagerPool.Active.Count; i++)
            {
                if (AudioManagerPool.Active[i].clip.name.Equals(clip))
                    return true;
            }
